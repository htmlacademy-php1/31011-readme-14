...
function get_posts($link, $where, $order, $limit = '')
{
    ...
    $sql = <<<SQL
        SELECT p.id, u.id user_id, u.login, u.email, u.avatar, c.type, p.header, p.post, p.date, u.date reg_date,
            p.author_quote, p.image_link, p.video_link, p.site_link, p.view, COUNT(DISTINCT p2.id) repost,
            COUNT(DISTINCT com.id) comments_count, COUNT(DISTINCT l.user_id) likes_count,
            COUNT(DISTINCT s.user_id) subscribed, COUNT(DISTINCT p1.id) posts, COUNT(DISTINCT s1.user_id) me_subscribed, p.post_id_original
        FROM `posts` p
        ...
        LEFT JOIN `posts` p2 ON p.id = p2.post_id_original
...
function get_posts_by_subscribed($link, $where, $order, $user_id, $limit = '')
{
    ...
    $sql = <<<SQL
        SELECT p.id, u.id user_id, u.login, u.email, u.avatar, c.type, p.header, p.post, p.date, u.date reg_date,
            ...
            COUNT(DISTINCT s1.user_id) me_subscribed, COUNT(DISTINCT p2.id) repost, s1.user_id suserid
        FROM `posts` p
        INNER JOIN `users` u ON p.user_id = u.id
        INNER JOIN `content_types` c ON p.type_id = c.id
        LEFT JOIN `comments` com ON p.id = com.post_id
